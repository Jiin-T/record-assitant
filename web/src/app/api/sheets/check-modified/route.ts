...
export async function POST(request: Request) {
  try {
    ...
    // 1. 자율/진로수정 시트에서 해당 활동의 수정된 데이터 찾기
    ...
    // 2. 추가문구수정 시트에서 해당 활동 찾기 (추가된 문구인 경우)
    if (!isModified) {
    ...
    return NextResponse.json({ 
      isModified,
      message: isModified ? "수정된 데이터가 있습니다." : "수정된 데이터가 없습니다."
    });

  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : "알 수 없는 오류가 발생했습니다.";
    console.error("Check Modified Error:", error);
